 ...
 */
void nop_print_accept(int set)
{
	printk(KERN_DEBUG "nop_test_accept flag set to %d: we accept."
		" Now cat trace_options to see the result\n", set);
}

void nop_print_refuse(int set)
{
	printk(KERN_DEBUG "nop_test_refuse flag set to %d: we refuse."
		" Now cat trace_options to see the result\n", set);
}

int nop_set_flag_rs(struct trace_array *tr, u32 old_flags, u32 bit, int set);

static int nop_set_flag(struct trace_array *tr, u32 old_flags, u32 bit, int set)
{
	return nop_set_flag_rs(tr, old_flags, bit, set);
}
